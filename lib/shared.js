...
/**
 * package.json 包对象(含 name,version,homepage);
 * >查看定义:@see {@link PK}、{@link dnsLookup}、{@link dnsResolve}
 */
const PK = (({ name, version, homepage }) => ({ name, version, homepage }))(pkg),
    DNS_TTL = 5 * 60 * 1000, DNS_MAP = new Map();
/** @type {Function} DNS 查找函数 */
const dnsLookup = dns.lookup;
/** @type {Function} DNS 解析函数 */
const dnsResolve = dns.resolve,
    ...
    */
    _callbackErr = (state = {}, connection, callback, err, transport) => {
...
/**
 * 解析连接URL为结构化配置对象
 * >查看定义:@see {@link parseConnectionUrl}
 ...
 * >查看定义:@see {@link initSmtpConstructor}
 * @param {object} instance - SMTP传输对象实例
 * @param {object} options - 配置选项
 * @param {function} SmtpC - SMTP连接类
 * @param {string} defaultComponent - 默认组件名称
 */
const initSmtpConstructor = (instance, options, SmtpC, defaultComponent) => {
...
/**
 * 处理连接错误和结束事件的通用逻辑
 * >查看定义:@see {@link setupConnectionHandlers}
 * @param {Object} state 状态对象
 * @param {Object} connection 连接对象
 * @param {Function} callback 回调函数
 ...
 * @returns {Object} 清理函数
 */
const setupConnectionHandlers = (state, connection, callback, transport) => {
...
/**
 * 执行 SMTP 认证的通用逻辑
 * >查看定义:@see {@link performSmtpAuthentication}
 * @param {Object} state 状态对象
 * @param {Object} connection 连接对象
 ...
 */
const performSmtpAuthentication = (state, connection, auth, options, callback, transport, onSuccess) => {
    if (state.returned) return;
    ...
    if (auth && (allowsAuth || forceAuth)) {
        connection.login(auth, err => {
            ...
            if (state.returned) return;
            err ? _callbackErr(state, connection, callback, err, transport) : onSuccess();
        });
    }
    // 否则,如果没有认证信息,则返回提供认证错误
    else if (!auth && allowsAuth && forceAuth) {
        const err = new Error('未提供认证信息');
        err.code = 'NoAuth', _callbackErr(state, connection, callback, err, transport);
    }
...
/**
 * 创建认证配置的通用逻辑
 * >查看定义:@see {@link createAuthConfig}
 * @param {Object} authData 认证数据
 * @param {Object} logger 日志记录器
 * @param {Object} mailer 邮件器实例（可选）
 * @returns {Object|string} 认证配置对象或空字符串
 */
const createAuthConfig = (authData, logger, mailer = null, OAUTH2) => {
    if (!authData) return '';
...
/**
 * 发送邮件的通用预处理逻辑
 * >查看定义:@see {@link prepareMessageForSending}
 ...
 */
const prepareMessageForSending = (mail, logger, connectionId = '', options = {}, done) => {
...
/**
 * 处理发送结果的通用逻辑
 * >查看定义:@see {@link handleSendResult}
 * @param {Object} result 发送结果
 * @param {Function} callback 回调函数
 * @param {Object} logger 日志记录器
 */
const handleSendResult = (result, callback, logger) => {
...
/**
 * 验证SMTP配置的通用函数
 * >查看定义:@see {@link verifySmtp}
 * @param {Object} transport 传输对象 (SmtpPool 或 SmtpTransport)
 * @param {Function} callback 回调函数
 * @param {Function} SmtpC... SMTP连接类
 * @param {Object} auth 认证信息
 */
const verifySmtp = (transport, callback, SmtpC, auth = null) => {
    // 如果没有提供回调，则创建Promise
    const promise = !callback ? new Promise((resolve, reject) => callback = callbackPromise(resolve, reject)) : null;
    ...
    getSocket(transport.options, (err, socketOp) => {
        ...
        connection.once('error', err => {
            ...
            if (state.returned) return;
            _callbackErr(state, connection, callback, new Error('连接已关闭'), transport);
        });
