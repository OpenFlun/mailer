 ...
 */
const encodeWords = (value, mimeWordEncoding = "Q", maxLength = 0, encodeAll) => {
 ...
 * >查看定义:@see {@link buildHeaderValue}
 ...
 */
const buildHeaderValue = structured => {
    const paramsArray = [];
 ...
 * >查看定义:@see {@link parseHeaderValue}
 * - 示例:
 * ```js
...
/**
 * 根据MIME类型检测文件扩展名;
 * >查看定义:@see {@link detectExtension}
 * @param {string} mimeType - 要检测的MIME类型
 * @returns {string} 检测到的文件扩展名
 */
const detectExtension = (mimeType = '') => {
    // 如果MIME类型为空，返回默认扩展名
...
/**
 • 折叠长行,适用于折叠头行(afterSpace=false)和流文本(afterSpace=true);
 * >查看定义:@see {@link foldLines}
 ...
 */
const foldLines = (str = '', lineLength = 76, afterSpace) => {
    // 计算调整长度
    const calculateAdjustment = (match, retainSpace) => {
